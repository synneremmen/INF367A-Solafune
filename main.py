...
from utils.evaluation import run_evaluation, f1_from_polygons
...
def main():
    subset = False
    model = UNet_Light().to(DEVICE)# SimpleConvNet().to(DEVICE) # UNet().to(DEVICE) # UNet_Light().to(DEVICE) # UNetResNet18().to(DEVICE)
    ...
    if os.path.exists(MODEL_PATH):
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=0.001) # endret lr fra 0.001 til 0.0001
